...
// Función para construir el índice FM
std::vector<Suffix> buildFMIndex(const std::string& text) {
    int n = text.size();
    std::vector<Suffix> suffixes(n);

    // Crear sufijos y almacenar sus índices
    for (int i = 0; i < n; i++) {
        suffixes[i].index = i;
        suffixes[i].suffix = text.substr(i) + text.substr(0, i);
    }

    // Ordenar sufijos lexicográficamente
...
// Función de búsqueda en el índice FM
int searchFMIndex(const std::string& pattern, const std::vector<Suffix>& fmIndex) {
    int m = pattern.size();
    int count = 0;

    for (const Suffix& suffix : fmIndex) {
        if (suffix.suffix.substr(0, m) == pattern) {
            count++;
        }
    }
...
void mide_FMIndex() {
    ...
    std::vector<Suffix> fmIndex = buildFMIndex(text);
    int occurrences = searchFMIndex(pattern, fmIndex);

    std::cout << "El patrón '" << pattern << "' aparece " << occurrences << " veces en el texto." << std::endl;
